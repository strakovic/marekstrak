...
export default function DashboardSwitcher({ items, className }: Props) {
  ...
  return (
    <div className={cn("relative w-full", className)}>
      {/* Container card */}
      <div className="relative rounded-3xl border border-black/5 bg-white/60 shadow-[0_10px_30px_rgba(0,0,0,0.05)] backdrop-blur-md dark:border-white/10 dark:bg-white/5">
        {/* Segmented control */}
        <div className="flex w-full justify-center px-4 pt-4">
          <div className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/70 p-1 backdrop-blur-sm dark:border-white/10 dark:bg-white/10">
            {items.map((it) => {
